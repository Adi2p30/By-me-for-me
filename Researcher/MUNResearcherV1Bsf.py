...
def makespeech(information, portfolio):
    ...
    print(
        "Write an MUN speech as "
        + portfolio
        + " on these topics "
        + information
        + " INCORPORATE AND ADD THE FOLLOWING POINTS (paste bard output here)"
    )
...
def getROP():
    ...
    print(
        "Yet this does not mean that this doesn't affect the chair's mentality about participation in the committee."
    )
    print(
    ...
    print(
        "If there is an unmoderated caucus raised, then it will always be voted upon first."
    )
    print(
        "Though this is not always followed, and chairs usually decide the order upon their discretion.\n"
    )
    ...
    print(
        "ii. It also may be used to state out a Factual Inaccuracy, which could lead to a lot of marks."
    )
    ...
    print(
        "7. Mandate:- The expected result and discussion method of a particular committee\n"
    )

    print(
        "8. Chits:- Used for communication informally to other delegates in a written format"
    )
